#let mythmere(
  ...
  let random-table(columns: 2, uniform: true, ..items) = {
    items = items.pos()
    let result = ()
    let cum_prob = 0
    let i = 0
    while i < items.len() {
      if uniform {
        ...
        if prob == 0 {
          ...
          result.push([#(cum_prob + 1)--#(cum_prob + prob)])
        }
        cum_prob += prob
        i += 1
      }
      let col = 1
      while i < items.len() and col < columns {
        let cell = items.at(i)
        result.push(cell)
        col += if (
          type(cell) == content
